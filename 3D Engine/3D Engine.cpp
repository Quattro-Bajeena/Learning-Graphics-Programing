...
struct vec3d {
	...
	float y = 0;
	float z = 0;
	float w = 1; //Need a 4th term to perform sensible matrix vector multiplications
...
private:
    mesh meshCube;
	mat4x4 matProj; //matrix that converts from view sapce to screen sapce

	vec3d vCamera; //location of cameara in world sapce
	vec3d vLookDir; //direction vector along he direction camera pints

	float fYaw; //fps camara rotation in XZ plane
	float fTheta; //spins the world transform
	...
	mat4x4 Matrix_MakeProjection(float fFovDegrees, float fAspectRatio, float fNear, float fFar) {
		float fFovRad = 1.f / tanf(fFovDegrees * 0.5f / 180.f * 3.14159f);
		mat4x4 matrix;
...
public:
    ...
    bool OnUserUpdate(float fElapsedTime) override {
		...
		mat4x4 rot = Matrix_MakeRotationY(0.5 * 3.14159f);
		vec3d vLookRight = Matrix_MultiplyVector(rot, vForward);
		vec3d vRight = Vector_Mul(vLookRight, 32.0f * fElapsedTime);
		...
		mat4x4 matTrans;
		matTrans = Matrix_MakeTranslation(0.0f, 0.0f, 5.f);
		...
		for (auto tri : meshCube.tris) {
			...
			//take a cross product of lines to get normal to riangle surface
			normal = Vector_CrossProduct(line1, line2);

			normal = Vector_Normalize(normal);

			//Get Ray from tirnagle to camera
			...
			if (Vector_DotProduct(normal, vCameraRay) < 0.f) {
				...
				//How aligned are light directon and triangle surface normal
				float dp = max(0.1f, Vector_DotProduct(light_direction, normal));

				
				//Chose console colors (much easier with RGB)
				...
				for (int n = 0; n < nClippedTriangles; n++) {
					...
					triProjected.t[0].w = 1.f / triProjected.p[0].w;
					triProjected.t[1].w = 1.f / triProjected.p[1].w;
					triProjected.t[2].w = 1.f / triProjected.p[2].w;
					

					//Scale into view, we moved the normalisng into cartesion sapce
					//out of the matrix.vector function from previous videos, so
					//do this manuall
					...
					//offset verts into visible normlised space
			...
			for (int plane = 0; plane < 4; plane++) {
				...
				while (nNewTriangles > 0) {
					...
					switch (plane) {
						...
						case 1: nTrisToAdd = Triangle_ClipAgainstPlane({ 0.f, (float)ScreenHeight() - 1, 0.f }, { 0.f, -1.f, 0.f }, test, clipped[0], clipped[1]); break;
						case 2: nTrisToAdd = Triangle_ClipAgainstPlane({ 0.f, 0.f, 0.f },						{ 1.f, 0.f, 0.f }, test, clipped[0], clipped[1]); break;
						case 3: nTrisToAdd = Triangle_ClipAgainstPlane({ (float)ScreenWidth(),0.f,  0.f },		{ -1.f, 0.f, 0.f }, test, clipped[0], clipped[1]); break;
					}
